							...
							<form name="myformupload" id="myformupload" class="form-horizontal" enctype="multipart/form-data" method="post" >
								<div class="box-body">
			                        ...
			                        <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <textarea class="form-control" name="BOQH_DESC"  id="BOQH_DESC" style="height:70px"></textarea>
                                        </div>
                                    </div>
...
<script>
    ...
    $('#example').DataTable( {
		...
		"autoWidth": true,
		"filter": true,
        "ajax": "<?php echo site_url('c_comprof/c_bUd93tL15t/get_AllDataHIST/?id='.$PRJCODE)?>",
        "type": "POST",
	...
	$(document).ready(function() {
		$('#myformupload').submit(function(e){
			...
			var BOQH_DESC = document.getElementById('BOQH_DESC').value;
			...
			$.ajax({
				...
				success: function(data)
				{
					...
					document.getElementById('BOQH_DESC').value	= '';
		...
		.then((willDelete) => {
			if (willDelete) 
			{
				...
				.then((value) => {
					...
					var IMPCODE = document.getElementById('BOQH_CODE'+row).value;
					var collID	= collID1;
					...
					$("#myiFrame")[0].contentWindow.IMP_TYPE.value 		= 'BOQ';
					butSubm.submit();
				});
			} 
			else 
