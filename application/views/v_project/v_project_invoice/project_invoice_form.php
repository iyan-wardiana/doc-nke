...
else
{
	...
	$PINV_KWITNO 		= $default['PINV_KWITNO'];
	$PINV_FAKNO 		= $default['PINV_FAKNO'];
	$PINV_INVOICE 		= $default['PINV_INVOICE'];
	...
	$PINV_TAXABLE 		= $default['PINV_TAXABLE'];
	
	$PINV_TOTPROGRESS		= $PINV_PROGVAL + $PINV_VALADD + $PINV_MATVAL;	
	$PINV_TOTPROGRESSPPn	= $PINV_PPNPERC * $PINV_TOTPROGRESS;
	$PINV_TOTPROGRESS		= $PINV_TOTPROGRESS + $PINV_TOTPROGRESSPPn;	
	$PINV_TOTPROGRESSNONPPn	= $PINV_TOTPROGRESS * 100 / ($PINV_PPNPERC * $PINV_PPNPERC);
	...
	$PINV_PAYAKUMPPn		= round($PINV_PPNPERC * $PINV_PAYAKUM / 100);
		      	...
		      	<form name="frm" method="post" action="<?php echo $form_action; ?>" enctype="multipart/form-data">
					<div class="col-md-6">
						<div class="box box-primary">
							...
							<div class="box-body">
					            <input type="hidden" name="PINV_PPNPERC" id="PINV_PPNPERC" value="<?php echo $PINV_PPNPERC; ?>">
	                        	...
	                        	<div class="col-sm-6">
	                                <div class="form-group">
				                        ...
				                        <input type="hidden" class="form-control" name="PINV_CODE1" id="PINV_CODE1" value="<?php echo $DocNumber; ?>" disabled>
	                                </div>
	                            </div>
	                        	...
	                        	<div class="col-sm-6">
	                                <div class="form-group">
	                                    <label for="inputName">No. Invoice</label>
	                                	<input type="text" name="PINV_INVOICE" id="PINV_INVOICE" value="<?php echo $PINV_INVOICE; ?>" class="form-control">
	                                </div>
	                            </div>
	                        	...
	                        	<div class="col-sm-6">
	                                <div class="form-group">
	                                	...
	                                	<input type="text" name="PINV_KWITNO" id="PINV_KWITNO" value="<?php echo $PINV_KWITNO; ?>" class="form-control">
	                                </div>
	                            </div>
	                        	...
	                        	<div class="col-sm-6">
	                                <div class="form-group">
	                                    <label for="inputName">No. Seri Pajak</label>
	                                	<input type="text" name="PINV_FAKNO" id="PINV_FAKNO" value="<?php echo $PINV_FAKNO; ?>" class="form-control">
	                                </div>
	                            </div>
					...
					<div class="col-md-6">
						<div class="box box-warning">
							...
							<div class="box-body">
	                        	...
	                        	<div class="col-sm-4">
	                                <div class="form-group">
	                                	<label for="inputName"><!-- PPn -->&nbsp;</label>
		                          		<?php
					                        $PRJCOST_PPN 	= round($PRJCOST * $PINV_PPNPERC / 100);
	                        	...
	                        	<div class="col-sm-2">
	                                <div class="form-group">
	                                	<label for="inputName" title="Tanpa Tax maka nilai PPn akan ada di 2 sisi, Debet dan Kredit">Tanpa PPn?</label>
		                          		<select name="PINV_TAXABLE" id="PINV_TAXABLE" class="form-control select2">
											<option value=""> --- </option>
											<option value="1"<?php if($PINV_TAXABLE == 1) { ?> selected <?php } ?>>No</option>
											<option value="0"<?php if($PINV_TAXABLE == 0) { ?> selected <?php } ?>>Ya</option>
										</select>
	                                </div>
	                            </div>
...
<script>
	...
	function getDetail(strItem)
	{
		...
		var PINV_PPNPERC	= parseFloat(document.getElementById('PINV_PPNPERC').value);
		...
		if(pageFrom == 'MC')
		{
			...
			// Nilai PPn Prestasi Saat ini - not use
				PINV_PAYAKUMPPn			= Math.round(parseFloat(PINV_PPNPERC * PINV_PAYAKUM));
		...
		else if(pageFrom == 'MCG')
		{
			...
			// OK - Nilai Prestasi
				...
				MC_PROGVAL			= parseFloat(MC_PROGVAL);
				MC_PROGVALPPn		= parseFloat(Math.round(PINV_PPNPERC * MC_PROGVAL));			// PPn Nilai Prestasi
				document.getElementById('PINV_PROGVAL1').value 		= doDecimalFormatxx(RoundNDecimal(parseFloat(Math.abs(MC_PROGVAL)),2));
			...
			// OK - Pekerjaan Tambahan
				MC_VALADD			= parseFloat(MC_VALADD);
				MC_VALADDPPn		= parseFloat(Math.round(PINV_PPNPERC * MC_VALADD));			// PPn Nilai Pekerjaan Tambah
				document.getElementById('PINV_VALADD').value 		= MC_VALADD;
			...
			// OK - Nilai Termasuk Pekerjaan Tambah
				...
				PINV_TOTPROGRESSPPn		= parseFloat(Math.round(PINV_PPNPERC * PINV_TOTPROGRESS));	// PPn Nilai Termasuk Pekerjaan Tambah
			...
			// OK - Nilai Uang Muka
				...
				MC_DPVALPPn			= parseFloat(Math.round(PINV_PPNPERC * MC_DPVAL));
				document.getElementById('PINV_DPVALPPn1').value 	= doDecimalFormatxx(RoundNDecimal(parseFloat(Math.abs(MC_DPVALPPn)),2));
				document.getElementById('PINV_DPVALPPn').value 		= MC_DPVALPPn;
			...
			// Nilai PPn Prestasi Saat ini - not use
				PINV_PAYAKUMPPn			= Math.round(parseFloat(PINV_PPNPERC * MC_TOTVAL));
				document.getElementById('PINV_PAYAKUMPPn').value 	= PINV_PAYAKUMPPn;
				//swal(PINV_PAYAKUMPPn)
		...
		if(pageFrom == 'SIC')
		{
			...
			// OK - Nilai Prestasi
				...
				SIC_PROGVALPPn		= parseFloat(Math.round(PINV_PPNPERC * SIC_PROGVAL));			// PPn Nilai Prestasi
    ...
    function getDPValue(thisVal)
    {
        ...
        PINV_DPVAL			= Math.round(DPPer * PRJCOST / 100);					//	Nilai Porject Tanpa PPn
        PINV_DPPPnVAL		= Math.round(PINV_DPVAL * PINV_PPNPERC / 100);				//	Nilai PPn dari DP
        ...
        document.getElementById('PINV_DPVAL').value 	= PINV_DPVAL;
        document.getElementById('PINV_DPVAL1').value 	= doDecimalFormatxx(RoundNDecimal(parseFloat(Math.abs(PINV_DPVAL)), 2));
        document.getElementById('PINV_DPVALPPn').value 	= PINV_DPPPnVAL;
        document.getElementById('PINV_DPVALPPn1').value = doDecimalFormatxx(RoundNDecimal(parseFloat(Math.abs(PINV_DPPPnVAL)),2));
		
		// Nilai Prestasi (Sebelum + PPn)
			PINV_TOTVAL		= parseFloat(PINV_DPVAL);
		
		// Nilai yang diajukan untuk dibayar plus PPn
			PINV_TOTVALnPPN	= parseFloat(PINV_DPVAL + PINV_DPPPnVAL);

		// TOTAL AKUM
			PINV_RETCUT		= document.getElementById('PINV_RETCUTCUR').value;
			PINV_PAYAKUM 	= parseFloat(PINV_DPVAL)  - parseFloat(PINV_RETCUT);
		...
		// TOTAL AKUM PPN
			PINV_PAYAKUMPPn = parseFloat(PINV_PAYAKUM * PINV_PPNPERC / 100);
	...
	function submitForm(value)
	{
		...
		var PINV_KWITNO	= document.getElementById('PINV_KWITNO').value;
		var PINV_FAKNO	= document.getElementById('PINV_FAKNO').value;
		var PINV_INVOICE	= document.getElementById('PINV_INVOICE').value;
		...
		var TAXABLE		= document.getElementById('PINV_TAXABLE').value;
		var PINV_OWNER 	= document.getElementById('PINV_OWNER').value;
        var ownIns 		= document.getElementById('OWN_INST').value;
        
        if(PINV_MANNO == '')
        {
        	swal('Nomor Invoice tidak boleh kosong',
        	{
        		icon:"warning"
        	})
        	.then(function()
        ...
        if(PINV_KWITNO == '')
        {
        	swal('Nomor Kwitansi tidak boleh kosong',
        	{
        		icon:"warning"
        	})
        	.then(function()
    		{
    			swal.close();
        		document.getElementById('PINV_KWITNO').focus();
    		});
        	return false;
        }
        ...
        /*if(PINV_FAKNO == '')
        {
        	swal('Nomor Seri Faktur tidak boleh kosong',
        	{
        		icon:"warning"
        	})
        	.then(function()
    		{
    			swal.close();
        		document.getElementById('PINV_FAKNO').focus();
    		});
        	return false;
        }*/
        ...
        if(PINV_INVOICE == '')
        {
        	swal('Nomor Jurnal tidak boleh kosong',
        	{
        		icon:"warning"
        	})
        	.then(function()
    		{
    			swal.close();
        		document.getElementById('PINV_INVOICE').focus();
    		});
        	return false;
        }
		...
		if(PINV_CATX == 2)
		{
			...
			if(PINV_SOURCE1 == '')
			{
				swal('Silahkan pilih nomor MC yang akan ditagihkan.',
				{
					icon:"warning"
				});
				return false;
		...
		if(PINV_MANNO == '')
		{
			swal('Nomor faktur tidak boleh kosong',
			{
				icon:"warning"
			});
			document.getElementById('PINV_MANNO').focus();
		...
		if(PINV_CATX != 1 && PINV_STEP == 0)
		{
			swal('Silahkan pilih tahapan MC',
			{
				icon:"warning"
			});
		...
		if(TAXABLE == '')
		{
			swal('Anda belum menentukan apakah faktur ini Tanpa PPn atau disertakan PPn?',
			{
				icon:"warning"
			});
			return false;
		...
		if(CatSelected != 1)
		{
			...
			if(PINV_PROG == 0)
			{
				swal('Persentase progress tidak boleh kosong / nol',
				{
					icon:"warning"
				});
		...
		if(isEdit == 'N')
		{
			swal('Maaf ... Anda tidak bisa mengubah dokumen ini.',
			{
				icon:"warning"
			});
			return false;
