...
if($task == 'add')
{
	...
	$PROP_CODE 		= "PDS.$PRJCODE.$lastPatt";
	$TRXTIME 		= date('ymdHis');
...
<html>
	...
	<?php
		...
		$secAddURL	= site_url('c_project/c_budprop/add/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
		$secGenCode	= base_url().'index.php/c_project/c_budprop/genCode/'; // Generate Code
		$secGetJID	= base_url().'index.php/c_project/c_budprop/getJID/'; // Generate Code
		$secGetPPn	= base_url().'index.php/c_project/c_budprop/getPPN/'; // Generate Code
		$secGetPPh	= base_url().'index.php/c_project/c_budprop/getPPH/'; // Generate Code
										...
										<?php
											// START : FOR ALL APPROVAL FUNCTION
												...
												if($task == 'add')
												{
													...
													{
														?>
															<select name="PROP_STAT" id="PROP_STAT" class="form-control select2" <?php if($disButton == 1){ ?> disabled <?php } ?>>
												...
												else
												{
													...
													if($canApprove == 1)
													{
														if($PROP_STAT == 1 || $PROP_STAT == 4)
														{
															//$disButton	= 1;
															?>
																<select name="PROP_STAT" id="PROP_STAT" class="form-control select2" >
																	<option value="1">New</option>
																	<option value="2">Confirm</option>
																</select>
															<?php
														}
														elseif($PROP_STAT == 2 || $PROP_STAT == 7)
														{
															//$disButton	= 0;
															if($canApprove == 0)
																$disButton	= 1;
															
															$sqlCAPPHE	= "tbl_approve_hist WHERE AH_CODE = '$PROP_NUM' AND AH_APPROVER = '$DefEmp_ID'";
															...
															?>
																<select name="PROP_STAT" id="PROP_STAT" class="form-control select2" onChange="selStat(this.value)" >
																	...
																	<option value="3"<?php if($PROP_STAT == 3) { ?> selected <?php } ?> >Approved</option>
																	<option value="4"<?php if($PROP_STAT == 4) { ?> selected <?php } ?> >Revising</option>
																	<option value="5"<?php if($PROP_STAT == 5) { ?> selected <?php } ?> >Rejected</option>
																	<option value="6"<?php if($PROP_STAT == 6) { ?> selected <?php } ?> disabled>Closed</option>
																	<option value="7"<?php if($PROP_STAT == 7) { ?> selected <?php } ?>>Waiting</option>
																	<option value="9"<?php if($PROP_STAT == 9) { ?> selected <?php } ?> disabled>Void</option>
																</select>
															<?php
														}
														elseif($PROP_STAT == 3)
														...
														elseif($PROP_STAT == 2 || $PROP_STAT == 7)
														{
															...
															?>
																<select name="PROP_STAT" id="PROP_STAT" class="form-control select2" onChange="selStat(this.value)" >
																	<option value="1"<?php if($PROP_STAT == 1) { ?> selected <?php } ?> disabled>New</option>
																	<option value="2"<?php if($PROP_STAT == 2) { ?> selected <?php } ?>>Confirm</option>
																	<option value="3"<?php if($PROP_STAT == 3) { ?> selected <?php } ?> disabled>Approved</option>
																	<option value="4"<?php if($PROP_STAT == 4) { ?> selected <?php } ?> disabled>Revising</option>
																	<option value="5"<?php if($PROP_STAT == 5) { ?> selected <?php } ?> disabled>Rejected</option>
																	<option value="6"<?php if($PROP_STAT == 6) { ?> selected <?php } ?> disabled>Closed</option>
																	<option value="7"<?php if($PROP_STAT == 7) { ?> selected <?php } ?> >Waiting</option>
		                ...
		                <div class="form-group">
		                    ...
		                    <div class="col-sm-9">
		                        <?php
									...
									// if(($PROP_STAT == 1 || $PROP_STAT == 2 || $PROP_STAT == 4) && $disButton == 0)
									// 	$btnShow = 1;
									// else
									// 	$btnShow = 0;
									...
									$btnShow = 0;
									if(($PROP_STAT == 1 || $PROP_STAT == 4) && $disButton == 0)
									{
										if($ISCREATE == 1)
											$btnShow = 1;
									}
								...
								<div class="row">
							    	<div class="col-md-12">
						              	<ul class="nav nav-tabs">
						                    ...
						                    <li id="li4">
						                    	<a href="#itm4" data-toggle="tab" onClick="setType(4)">Overhead</a>
						                    </li>
						                </ul>
							            <div class="box-body">
			                                            ...
			                                            <table id="example1" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
			                                                <thead>
			                                                    <tr>
											                        ...
											                        <th width="62%" style="text-align: center;" nowrap><?php echo $Description; ?></th>
							            	...
							            	<div class="<?php echo $Active4; ?> tab-pane" id="itm4" style="display: none;">
		                                        <div class="form-group">
		                                        	<form method="post" name="frmSearch4" id="frmSearch3" action="">
		                                        		<div class="search-table-outter">
...
<script>
	...
	function add_item(strItem)					// OK
	{
		...
		// FIELD LIST
			...
			var JOBPARDESCV	= '<div style="font-style: italic;"><i class="text-muted fa fa-chevron-circle-right"></i>&nbsp;&nbsp;'+JOBPARDESC+'</div>';
		...
		// ITM_NAME
			...
			objTD.innerHTML = ''+ITM_NAME+JOBPARDESCV+'';
