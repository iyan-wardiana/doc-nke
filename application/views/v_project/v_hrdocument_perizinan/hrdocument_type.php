				...
				<div class="search-table-outter">
			      	<table id="example1" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
						...
						<?php
							...
							$sqlTypeC		= "tbl_document WHERE doc_code IN ('D0242') AND isHRD = 1 AND doc_level = 3";
							$resTypeC		= $this->db->count_all($sqlTypeC);
					
							$sqlType		= "SELECT doc_code, doc_parent, doc_name FROM tbl_document WHERE doc_code  
												IN ('D0242')
												AND isHRD = 1 
												AND doc_level = 3";
							...
							if($resTypeC >0)
							{
								foreach($resType as $rowType) :
									...
									$secGetDoc		= site_url('c_project/hrdocument_perizinan/get_last_ten_Type/?id='.$this->url_encryption_helper->encode_url($doc_code));
...
<script>
	$(document).ready(function() {
		$('#example1').DataTable({
			...
			"columnDefs": [	{ targets: [1,2], className: 'dt-body-center' },
