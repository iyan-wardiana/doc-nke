...
$MDEPT_CODE1	= 'JXXX';
$sqlMDC	= "SELECT B.MDEPT_CODE, B.POSS_PARENT FROM tbl_employee A 
            INNER JOIN tbl_position_str B ON B.POSS_CODE = A.Pos_Code
            WHERE Emp_ID = '$DefEmp_ID'";
$sqlMDC	= $this->db->query($sqlMDC)->result();
foreach($sqlMDC as $rowMDC) :
	$MDEPT_CODE1= $rowMDC->MDEPT_CODE;
	$POSS_PARENT= $rowMDC->POSS_PARENT;
endforeach;
if($MDEPT_CODE1 == '')
	$MDEPT_CODE1 = 'JXXX';
            ...
            <div class="box box-primary">
                ...
                <form name="frm1" method="post" action="<?php echo $form_action; ?>" enctype="multipart/form-data" onSubmit="return checkData();">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group" <?php if($MDEPT_CODE1 != 'JXXX') { ?> style="display: none;" <?php } ?>>
                        ...
                        <div class="form-group">
                            <select name="MB_TOG[]" id="MB_TOG" class="form-control select2" multiple="multiple" data-placeholder="&nbsp;&nbsp;&nbsp;Mail To Group" style="width: 100%;">
                                <?php
                                    $sqlMG  = "SELECT POSS_CODE, POSS_NAME FROM tbl_position_str WHERE POSS_PARENT = '$POSS_PARENT'";
                                    $sqlMG	= $this->db->query($sqlMG)->result();
                                    foreach($sqlMG as $rowMG) :
                                        $MG_CODE	= $rowMG->POSS_CODE;
                                        $MG_NAME	= $rowMG->POSS_NAME;
                                        ?>
