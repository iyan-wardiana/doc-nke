...
<?php
	...
	$secDraft_Del	= site_url('c_mailbox/c_mailbox/trash_mail_process_idx_I_all/?id='.$this->url_encryption_helper->encode_url($DefEmp_ID));
                ...
                <div class="box-body no-padding">
                    <ul class="nav nav-pills nav-stacked">
                        ...
                        <li class="active">
                            ...
                            <span class="label label-primary pull-right"><?php echo $countTrash; ?></span></a>
                        </li>
                    </ul>
                </div>
            </div>
                    ...
                    <div class="box">
                        <div class="box-body">
                            <div class="table-responsive mailbox-messages">
                            	<form method="post" name="sendDeleteAll" id="sendDeleteAll" class="form-del-all" action="<?php echo $secTrash_Del; ?>" style="display:none">		
                                    <table>
                                        <tr>
                                            <td></td>
                                            <td><a class="tombol-delete-all" id="delClassAll">Delete All</a></td>
                                        </tr>
                                    </table>
                                </form>
                            	<form method="post" name="sendDelete" id="sendDelete" class="form-del" action="" style="display:none">		
                                    <table>
                                        <tr>
                                            <td></td>
                                            <td><a class="tombol-delete" id="delClass">Delete</a></td>
                                        </tr>
                                    </table>
                                </form>
                                    ...
                                    <?php
                                        ...
                                        if($countTrash >0)
                                        {
                                            foreach($viewmail as $row) :
                                                $myNewNo 		= ++$i;
                                                $MBT_ID 		= $row->MBT_ID;
                                                $MBT_CODE 		= $row->MBT_CODE;
                                                $MBT_PARENTC	= $row->MBT_PARENTC;
                                                $MBT_DATE		= $row->MBT_DATE;
                                                $MBT_DATE1		= $row->MBT_DATE1;
                                                $MBT_READD		= $row->MBT_READD;
                                                $MBT_SUBJECT	= $row->MBT_SUBJECT;
                                                $MBT_FROM_ID	= $row->MBT_FROM_ID;
                                                $MBT_FROM		= $row->MBT_FROM;
                                                $MBT_TO_ID		= $row->MBT_TO_ID;
                                                $MBT_TO			= $row->MBT_TO;
                                                $MBT_TOV		= $row->MBT_TO;
                                                $MBT_MESSAGE	= $row->MBT_MESSAGE;
                                                $MBT_STATUS		= $row->MBT_STATUS;
                                                $MBT_FN1		= $row->MBT_FN1;
												
												//$TIME1 		= strtotime($MBT_DATE1);
												$TIME1_Y		= date("Y", strtotime($MBT_DATE1));
												$TIME1_M		= date("m", strtotime($MBT_DATE1));
												$TIME1_D		= date("d", strtotime($MBT_DATE1));
												$TIME1_H		= date("H", strtotime($MBT_DATE1));
												$TIME1_I		= date("i", strtotime($MBT_DATE1));
												$TIME1_S		= date("s", strtotime($MBT_DATE1));
												...
												echo "$MBT_DATE1 == $TIME1_F : $TIME2_F = $jumlah_hari : $jumlah_jam : $jumlah_menit<br>";*/
												...
												elseif($TIME_DIFF_H <= 48)
													...
													$TIME_DIFFD	= date_format(date_create($MBT_DATE), "d M");
												
												$secRead_Mail	= base_url().'index.php/c_mailbox/c_mailbox/read_mail_S/?id='.$MBT_ID;
                                                if($MBT_STATUS == 1)
												{
													if ($j==1) {
                                                ...
                                                ?>
                                                    ...
                                                    <td class="mailbox-name" nowrap>
                                                    	<a href="<?php echo $secRead_Mail; ?>">
															<?php
                                                                echo cut_text2 ("$MBT_TO", 16);
                                                            ?>
                                                        </a>
                                                    </td>
                                                    <td class="mailbox-subject" nowrap>
														<?php
                                                            echo cut_text2 ("$MBT_SUBJECT", 15);
                                                        ?>
                                                    </td>
                                                    <td class="mailbox-subject">
														<?php
                                                            echo cut_text2 ("$MBT_MESSAGE", 50);
                                                        ?>
                                                    </td>
                                                    ...
                                                    <?php
														echo $TIME_DIFFD;
										                $secDel_Mail	= base_url().'index.php/c_mailbox/c_mailbox/trash_mail_process_idx_T/?id='.$MBT_ID;
													?>
                                                    </td>
...
<script type="text/javascript">
	...
	$(document).ready(function()
	{
		...
		{
			var index_Mail	= "<?php echo $secTrash_Mail; ?>";
