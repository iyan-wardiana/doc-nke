...
class M_purchase_req extends CI_Model
{
	...
	function get_AllDataGRPC($PRJCODE, $PO_NUM, $PR_STAT, $search) // GOOD
	{
		$ADDQRY1 	= "";
		$ADDQRY2 	= "";
	...
	function get_AllDataITMOC($PRJCODE, $search) // GOOD
	{
		$sql = "tbl_joblist_detail A
				WHERE A.PRJCODE = '$PRJCODE' AND ITM_GROUP NOT IN ('M','T') AND A.ISLAST = 1 AND A.WBSD_STAT = 1 AND A.ITM_CODE != ''
					AND (A.JOBCODEID LIKE '%$search%' ESCAPE '!' OR A.ITM_UNIT LIKE '%$search%' ESCAPE '!'
				OR A.JOBDESC LIKE '%$search%' ESCAPE '!') ORDER BY A.ORD_ID";
		return $this->db->count_all($sql);
	}
	...
	function get_AllDataITMDETC($PRJCODE, $PR_NUM, $task, $search) // GOOD
	{
		$s_sql 	= "SELECT COUNT(*) AS TOT_PR
					...
					UNION ALL
					SELECT B.* FROM tbl_pr_detail_tmp B WHERE B.PR_NUM = '$PR_NUM' AND B.PRJCODE = '$PRJCODE'
	...
	function get_AllDataITMDETL($PRJCODE, $PR_NUM, $task, $search, $length, $start, $order, $dir) // GOOD
	{
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT DISTINCT A.PR_ID, A.PR_NUM, A.PR_CODE, A.PR_DATE, A.PRJCODE, A.DEPCODE, A.JOBCODEDET, A.JOBCODEID, A.JOBPARENT, A.JOBPARDESC,
							...
							A.PR_CVOL
						FROM tbl_pr_detail A WHERE A.PR_NUM = '$PR_NUM' AND A.PRJCODE = '$PRJCODE'
							AND (A.ITM_CODE LIKE '%$search%' ESCAPE '!' OR A.ITM_NAME LIKE '%$search%' ESCAPE '!'
							OR A.PR_DESC LIKE '%$search%' ESCAPE '!')
						UNION ALL
						SELECT DISTINCT B.PR_ID, B.PR_NUM, B.PR_CODE, B.PR_DATE, B.PRJCODE, B.DEPCODE, B.JOBCODEDET, B.JOBCODEID, B.JOBPARENT, B.JOBPARDESC,
							...
							B.PR_CVOL
						FROM tbl_pr_detail_tmp B WHERE B.PR_NUM = '$PR_NUM' AND B.PRJCODE = '$PRJCODE'
							AND (B.ITM_CODE LIKE '%$search%' ESCAPE '!' OR B.ITM_NAME LIKE '%$search%' ESCAPE '!'
