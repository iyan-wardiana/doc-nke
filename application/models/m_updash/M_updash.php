...
class M_updash extends CI_Model
{
	...
	function get_AllDataDWRC($DWR_EMPID, $Start_Date, $End_Date, $search, $length, $start) // GOOD
	{
		## Custom Field value
		...
		if($length == -1)
		{
			$s_sql 	= "SELECT COUNT(*) AS TOT_LKH
						FROM (SELECT A.* FROM tbl_lkh A WHERE A.DWR_EMPID = '$DWR_EMPID' AND DWR_DATE BETWEEN '$Start_Date' AND '$End_Date'
	...
	function get_AllDataDWRL($DWR_EMPID, $Start_Date, $End_Date, $search, $length, $start, $order, $dir) // GOOD
	{
		## Custom Field value
		//$searchByDATE 	= $this->input->post('DATE_SRC');
		...
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT A.* FROM tbl_lkh A WHERE A.DWR_EMPID = '$DWR_EMPID' AND DWR_DATE BETWEEN '$Start_Date' AND '$End_Date'
