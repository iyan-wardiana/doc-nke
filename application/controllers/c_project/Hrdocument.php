...
class Hrdocument  extends CI_Controller
{
	function __construct() // GOOD
	{
		parent::__construct();

		$this->load->model('m_project/m_hrdocument/m_hrdocument', '', TRUE);
 	...
 	public function index() // OK
	{
		...
		$url			= site_url('c_project/hrdocument/get1nd3x/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function get1nd3x()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data["secURL"] 	= "c_project/hrdocument/get_last_ten_GroupType/?id=";
	...
	function get_last_ten_GroupType()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// -------------------- START : SEARCHING METHOD --------------------
				...
				$data["url_search"] = site_url('c_project/hrdocument/f4n7_5rcH/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$data["MenuCode"] 	= 'MN294';
			$data['backURL'] 	= site_url('c_project/hrdocument/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$this->load->view('v_project/v_hrdocument/hrdocument_type', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function get_last_ten_projList($offset=0) // HOLD
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['secAddURL'] 			= site_url('c_project/hrdocument/add/?id='.$this->url_encryption_helper->encode_url($appName));
			$data['showIndex'] 			= site_url('c_project/hrdocument/index/?id='.$this->url_encryption_helper->encode_url($appName));
			$data['srch_url'] 			= site_url('c_project/hrdocument/get_last_ten_docpattern_src/?id='.$this->url_encryption_helper->encode_url($appName));			
			$data['moffset'] 			= $offset;
			$data['perpage'] 			= 20;
			$data['theoffset']			= 0;
			...
			$config['base_url'] 		= site_url('c_project/hrdocument/get_last_ten_projList');				
			...
			$this->load->view('v_project/v_hrdocument/project_planning', $data);
		}
		else
		{
			redirect('Auth');
	...
	function hr_documentlistx($offset=0) // HOLD
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['h3_title'] 			= 'HR Document';
			$data['main_view'] 			= 'v_project/v_hrdocument/hrdocumentlist';	
					
			$data['link'] 				= array('link_back' => anchor('c_project/hrdocument/','<input type="button" name="btnCancel" id="btnCancel" class="btn btn-danger" value="Back" />', array('style' => 'text-decoration: none;')));
			$data['PRJCODE'] 			= $PRJCODE;
			//$data['moffset'] 			= $offset;
			...
			$data["recordcount"] 		= $num_rows;			
			$config 					= array();
			$config['base_url'] 		= site_url('c_project/hrdocument/get_last_ten_projHRDOC1');
			$config["total_rows"] 		= $num_rows;			
			$config["per_page"] 		= 20;
			...
			$this->load->view('v_project/v_hrdocument/hrdocumentlist', $data);
		}
		else
		{
			redirect('Auth');
	...
	function get_last_ten_Type($offset=0) // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$cancel_url				= site_url('c_project/hrdocument/get_last_ten_GroupType/?id='.$this->url_encryption_helper->encode_url($appName));
			...
			$this->load->view('v_project/v_hrdocument/hrdocument', $data);
		}
		else
		{
			redirect('Auth');
	...
	function hr_documentlist() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$cancel_url			= site_url('c_project/hrdocument/get_last_ten_Type/?id='.$this->url_encryption_helper->encode_url($doc_parentA));
	...
	function do_upload() // OK
	{
		...
		$url			= site_url('c_project/Hrdocument/hr_documentlist/?id='.$this->url_encryption_helper->encode_url($DOCCODE));
		redirect($url);
	}
