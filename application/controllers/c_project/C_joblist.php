...
class C_joblist extends CI_Controller  
{
  	...
  	function get_AllDataJL() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$COLITM0 		= "";
				$secDelete 		= "";
				/*$s_GDET1 		= "tbl_joblist_detail WHERE JOBPARENT = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
				$r_GDET1 		= $this->db->count_all($s_GDET1);

				$r_GDET 		= $r_GDET1;
				if($r_GDET == 0)
				{
					$secDelete	= 	"<input type='hidden' name='urlDelJob".$noU."' id='urlDelJob".$noU."' value='".$delJobID."'>
								   	<label style='white-space:nowrap'>
										<a href='javascript:void(null);' class='btn bg-red btn-xs' onClick='deleteJL(".$noU.")' title='Hapus'>
				...
				{
					$s_ITM0 	= "SELECT ITM_CODE FROM tbl_joblist_detail WHERE JOBPARENT = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
					$r_ITM0 	= $this->db->query($s_ITM0)->result();
					foreach($r_ITM0 as $rw_ITM0):
						$ITM_CODE0	= $rw_ITM0->ITM_CODE;
						$COLITM0	= "$COLITM0, $ITM_CODE0";
					endforeach;
				}*/
				...
				if($ISLASTH == 0 && $ISLAST == 0)
				{
					// GET DEVIASI
						$TOTDET 	= $JOBCOST;
						$s_TDET 	= "SELECT SUM(A.ITM_BUDG) AS TOTDET FROM tbl_joblist_detail A
					...
					if($TOTDET != $JOBCOST)
					{
						...
						$secReSUM	= 	"<input type='hidden' name='urlReSUM".$noU."' id='urlReSUM".$noU."' value='".$ReSUMID."'>
											<div style='text-align:right;white-space:nowrap;'>
												...
												&nbsp;&nbsp;
												<span class='label label-".$STATCOL."' style='font-size:12px'>".number_format($JOBCOST,2)."</span>
											</div>";
					}
					else
					{
				...
				/*if($BOQ_JOBCOST == 0)
				{
					...
					if($BOQ_JOBCOST == 0)
					{
						...
						foreach($resTBUDG as $rowTBUDG) :
							$BOQ_JOBCOST	= $rowTBUDG->TOT_BUDGAM;
							$ADD_JOBCOST 	= $rowTBUDG->TOT_ADDAM;
							$ITM_USED_AM 	= $rowTBUDG->TOT_USEDAM;
						endforeach;
						if($BOQ_JOBCOST == 0)
						{
							$sqlTBUDG	= "SELECT SUM(ITM_BUDG) AS TOT_BUDGAM, SUM(ADD_JOBCOST) AS TOT_ADDAM, SUM(ITM_USED_AM) AS TOT_USEDAM
  	...
  	function get_AllDataRAPDET() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$s_00 			= "tbl_joblist_detail WHERE JOBPARENT = '$JOBPARENT' AND PRJCODE = '$PRJCODE'";
			$r_00 			= $this->db->count_all($s_00);

			if($num_rows == 0 && $r_00 > 1)
			{
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$JOBDESC 		= "-";
				$s_JDESC		= "SELECT JOBDESC FROM tbl_joblist WHERE JOBCODEID = '$JOBPARENT' AND  PRJCODE = '$PRJCODE' LIMIT 1";
				...
				// CHECK TRANSACTION
					...
					$s_trx 		= 	"SELECT COUNT(*) AS TOT_TRX FROM tbl_pr_detail WHERE PRJCODE = '$PRJCODE' AND JOBCODEID = '$JOBCODEID'
									...
									UNION
									SELECT COUNT(*) AS TOT_TRX FROM tbl_journaldetail WHERE proj_Code = '$PRJCODE' AND JOBCODEID = '$JOBCODEID'
									UNION
                ...
                $lockID 		= "$secLock~$PRJCODE~$JOB_NUM~$JOBCODEID~$JOBPARENT~$ITM_CODE~$ITM_NAME";
	...
	function delComp()
	{
		...
		$JOBCODEID 	= "";
		$s_00 		= "SELECT JOBCODEID FROM tbl_jobcreate_detail WHERE JOBD_ID = '$JOBD_ID' AND JOB_NUM = '$JOB_NUM' AND ITM_CODE = '$ITM_CODE'
	...
	function lockComp()
	{
		...
		$PRJ_NKE 	= $PRJ_HO;
		$s_00a 		= "SELECT PRJCODE FROM tbl_project WHERE isHO = 1";
		...
		// START : CHECK EXISTING JOB PROCEDURE
			$s_JID 	= "tbl_joblist_detail WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
			...
			if($r_JID > 0)
			{
				// START : UPDATE JOBLISTDETAIL
					...
					foreach($r_00 as $rw_00) :
						...
						$s_01 			= 	"UPDATE tbl_joblist_detail SET ITM_VOLM = '$ITM_RAPV', ITM_PRICE = '$ITM_RAPP',
						...
						// UPDATE ITEM BUDGET
							$TOTVOLBUDG = 0;
							$TOTAMNBUDG = 1;
							$s_02	= "SELECT SUM(B.ITM_VOLM) AS TOTVOLBUDG, SUM(B.ITM_BUDG) AS TOTAMNBUDG FROM tbl_joblist_detail B
											WHERE B.ITM_CODE = '$ITM_CODE' AND B.PRJCODE = '$PRJCODE'";
							$r_02 	= $this->db->query($s_02)->result();
							foreach($r_06 as $rw_02) :
						...
						// UPDATE STATUS ANALISA
							$DefEmp_ID 		= $this->session->userdata['Emp_ID'];
							$compName 		= "";
							$sqlEmp 		= "SELECT CONCAT(First_Name, ' ', Last_Name) AS compName, Log_Act FROM tbl_employee WHERE Emp_ID = '$DefEmp_ID'";
							...
							/*$s_04			= 	"UPDATE tbl_jobcreate_detail SET ISLOCK = 1, LOCKER_ID = '$DefEmp_ID', LOCKER_NM = '$compName', JOBCODEID = '$JOBID_DET'
													WHERE JOB_NUM = '$JOB_NUM' AND ITM_CODE = '$ITM_CODE' AND JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";*/
							$s_04			= 	"UPDATE tbl_jobcreate_detail SET ISLOCK = 1, LOCKER_ID = '$DefEmp_ID', LOCKER_NM = '$compName'
													WHERE JOB_NUM = '$JOB_NUM' AND ITM_CODE = '$ITM_CODE' AND JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
							$this->db->query($s_04);

						// START : UPDATE ALL HEADER IN HIERARKI
							$s_u100	= "UPDATE tbl_jobcreate_header SET JOB_RAPT = (SELECT SUM(A.ITM_TOTAL) AS TOTDET FROM tbl_jobcreate_detail A WHERE A.JOBPARENT = '$JOBPARENT'
												AND A.PRJCODE = '$PRJCODE'), JOB_RAPP = JOB_RAPT / IF(JOB_RAPV=0,1,JOB_RAPV)
										WHERE JOB_PARCODE = '$JOBPARENT' AND PRJCODE = '$PRJCODE'";
							$this->db->query($s_u100);

							$s_u1a0	= "UPDATE tbl_joblist SET JOBCOST = (SELECT SUM(A.ITM_BUDG) AS TOTDET FROM tbl_joblist_detail A WHERE A.JOBPARENT = '$JOBPARENT'
												AND A.PRJCODE = '$PRJCODE'), PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
										WHERE JOBCODEID = '$JOBPARENT' AND PRJCODE = '$PRJCODE'";
							$this->db->query($s_u1a0);

							$s_u1b0	= "UPDATE tbl_boqlist A, tbl_joblist B SET A.JOBCOST = B.JOBCOST, A.PRICE = A.JOBCOST / IF(A.JOBVOLM=0,1,A.JOBVOLM)
										WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
											AND B.JOBCODEID = '$JOBPARENT' AND B.PRJCODE = '$PRJCODE'";
							$this->db->query($s_u1b0);

							$s_u1c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
											A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
											A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
										WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
											AND B.JOBCODEID = '$JOBPARENT' AND B.PRJCODE = '$PRJCODE'";
							$this->db->query($s_u1c0);
							...
							foreach($r_P2 as $rw_P2) :
								...
								$s_u2c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
												A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
												A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
											WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
												AND B.JOBCODEID = '$JP2' AND B.PRJCODE = '$PRJCODE'";
								...
								foreach($r_P3 as $rw_P3) :
									...
									$s_u3c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
													A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
													A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
												WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
													AND B.JOBCODEID = '$JP3' AND B.PRJCODE = '$PRJCODE'";
									...
									foreach($r_P4 as $rw_P4) :
										...
										$s_u4c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
														A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
														A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
													WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
														AND B.JOBCODEID = '$JP4' AND B.PRJCODE = '$PRJCODE'";
										...
										foreach($r_P5 as $rw_P5) :
											...
											$s_u5c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
															A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
															A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
														WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
															AND B.JOBCODEID = '$JP5' AND B.PRJCODE = '$PRJCODE'";
											...
											foreach($r_P6 as $rw_P6) :
												...
												$s_u6c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
																A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
																A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
															WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
																AND B.JOBCODEID = '$JP6' AND B.PRJCODE = '$PRJCODE'";
												...
												foreach($r_P7 as $rw_P7) :
													...
													$s_u7c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
																	A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
																	A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
																WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
																	AND B.JOBCODEID = '$JP7' AND B.PRJCODE = '$PRJCODE'";
													...
													foreach($r_P8 as $rw_P8) :
														...
														$s_u8c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
																		A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
																		A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
																	WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
																		AND B.JOBCODEID = '$JP8' AND B.PRJCODE = '$PRJCODE'";
														...
														foreach($r_P9 as $rw_P9) :
															...
															$s_u9c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
																			A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
																			A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
																		WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
																			AND B.JOBCODEID = '$JP9' AND B.PRJCODE = '$PRJCODE'";
															...
															foreach($r_P10 as $rw_P10) :
																...
																$s_u10c0	= "UPDATE tbl_joblist_detail A, tbl_joblist B SET A.ITM_VOLM = IF(A.ITM_VOLM = 0, 1, A.ITM_VOLM),
																				A.ITM_BUDG = B.JOBCOST, A.ITM_PRICE = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM), A.ITM_LASTP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM),
																				A.ITM_AVGP = B.JOBCOST / IF(A.ITM_VOLM=0,1,A.ITM_VOLM)
																			WHERE A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
																				AND B.JOBCODEID = '$JP10' AND B.PRJCODE = '$PRJCODE'";
			...
			else
			{
				// START : INSERT PROCEDURE
					...
					// START : GET MAXIMUM ORD_ID DETAIL BERDASARKAN INDUK PEKERJAAN. KECUALI JIKA HEADER TSB BELUM MEMILIKI TURUNAN
						...
						{
							$s_03	= "SELECT MAX(ORD_ID) AS ORD_ID, MAX(Patt_Number) AS TOT_RW FROM tbl_joblist_detail
					...
					// START : INSERT INTO JOBLIST DETAIL
						...
						$s_ITM		= "SELECT ITM_UNIT, ITM_GROUP, ITM_KOEF, ITM_RAPV, ITM_RAPP, ITM_TOTAL
										FROM tbl_jobcreate_detail WHERE JOB_NUM = '$JOB_NUM' AND ITM_CODE = '$ITM_CODE'
											AND JOBPARENT = '$JOBPARENT'AND PRJCODE = '$PRJCODE'";
						...
						// INSERT INTO JOBDET
							...
							if($r_05a == 0)
							{
								$s_05b 		= "SELECT ITM_NAME, ITM_CATEG, ITM_TYPE, ITM_CURRENCY, ACC_ID, ACC_ID_UM, ACC_ID_SAL, STATUS, ISMTRL, ITM_LR
												FROM tbl_item WHERE PRJCODE = '$PRJ_NKE' AND ITM_CODE = '$ITM_CODE' LIMIT 1";
		...
		echo "Analisa RAP sudah kami proses. Silahkan cek di daftar RAP..! $s_P2 : $s_u1c0. $s_P3: $s_u2c0";
    }
	...
	function undoComp()
	{
		...
		// START : UPDATE PROCEDURE
			$ITM_BUDG = 0;
			$s_00 	= "SELECT ITM_BUDG FROM tbl_joblist_detail WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE' LIMIT 1";
			$r_00 	= $this->db->query($s_00)->result();
			foreach($r_00 as $rw_00) :
			...
			$s_02			= 	"DELETE FROM tbl_joblist_detail WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
			//$this->db->query($s_02);

			// UPDATE ITEM BUDGET
				$TOTVOLBUDG = 0;
				$TOTAMNBUDG = 1;
				$s_03	= "SELECT SUM(B.ITM_VOLM) AS TOTVOLBUDG, SUM(B.ITM_BUDG) AS TOTAMNBUDG FROM tbl_joblist_detail B
								WHERE B.ITM_CODE = '$ITM_CODE' AND B.PRJCODE = '$PRJCODE'";
				$r_03 	= $this->db->query($s_03)->result();
				foreach($r_03 as $rw_03) :
		...
		// UPDATE ALL HEADER IN HIERARKI
			$s_u2a0	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
							JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
						WHERE JOBCODEID = '$JOBPARENT' AND PRJCODE = '$PRJCODE'";
			$this->db->query($s_u2a0);

			$s_u2a0	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
							JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
						WHERE JOBCODEID = '$JOBPARENT' AND PRJCODE = '$PRJCODE'";
			$this->db->query($s_u2a0);

			$s_u2b0	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
							ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
							, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
						WHERE JOBCODEID = '$JOBPARENT' AND PRJCODE = '$PRJCODE'";
			$this->db->query($s_u2b0);
			...
			foreach($r_P2 as $rw_P2) :
				...
				$s_u2a	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
								JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
							WHERE JOBCODEID = '$JP2' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_u2a);

				$s_u2a	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
								JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
							WHERE JOBCODEID = '$JP2' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_u2a);

				$s_u2b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
								ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
								, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
							WHERE JOBCODEID = '$JP2' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_u2b);
				...
				foreach($r_P3 as $rw_P3) :
					$JP3 	= $rw_P3->JOBPARENT;
					$s_u3	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
									JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
								WHERE JOBCODEID = '$JP3' AND PRJCODE = '$PRJCODE'";
					$this->db->query($s_u3);

					$s_u3	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
									JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
								WHERE JOBCODEID = '$JP3' AND PRJCODE = '$PRJCODE'";
					$this->db->query($s_u3);

					$s_u3b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
									ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
									, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
								WHERE JOBCODEID = '$JP3' AND PRJCODE = '$PRJCODE'";
					$this->db->query($s_u3b);
					...
					foreach($r_P4 as $rw_P4) :
						$JP4 	= $rw_P4->JOBPARENT;
						$s_u4	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
										JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
									WHERE JOBCODEID = '$JP4' AND PRJCODE = '$PRJCODE'";
						$this->db->query($s_u4);

						$s_u4	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
										JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
									WHERE JOBCODEID = '$JP4' AND PRJCODE = '$PRJCODE'";
						$this->db->query($s_u4);

						$s_u4b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
										ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
										, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
									WHERE JOBCODEID = '$JP4' AND PRJCODE = '$PRJCODE'";
						$this->db->query($s_u4b);
						...
						foreach($r_P5 as $rw_P5) :
							$JP5 	= $rw_P5->JOBPARENT;
							$s_u5	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
											JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
										WHERE JOBCODEID = '$JP5' AND PRJCODE = '$PRJCODE'";
							$this->db->query($s_u5);

							$s_u5	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
											JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
										WHERE JOBCODEID = '$JP5' AND PRJCODE = '$PRJCODE'";
							$this->db->query($s_u5);

							$s_u5b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
											ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
											, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
										WHERE JOBCODEID = '$JP5' AND PRJCODE = '$PRJCODE'";
							$this->db->query($s_u5b);
							...
							foreach($r_P6 as $rw_P6) :
								$JP6 	= $rw_P6->JOBPARENT;
								$s_u6	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
												JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
											WHERE JOBCODEID = '$JP6' AND PRJCODE = '$PRJCODE'";
								$this->db->query($s_u6);

								$s_u6	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
												JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
											WHERE JOBCODEID = '$JP6' AND PRJCODE = '$PRJCODE'";
								$this->db->query($s_u6);

								$s_u6b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
												ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
												, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
											WHERE JOBCODEID = '$JP6' AND PRJCODE = '$PRJCODE'";
								$this->db->query($s_u6b);
								...
								foreach($r_P7 as $rw_P7) :
									$JP7 	= $rw_P7->JOBPARENT;
									$s_u7	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
													JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
												WHERE JOBCODEID = '$JP7' AND PRJCODE = '$PRJCODE'";
									$this->db->query($s_u7);

									$s_u7	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
													JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
												WHERE JOBCODEID = '$JP7' AND PRJCODE = '$PRJCODE'";
									$this->db->query($s_u7);

									$s_u7b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
													ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
													, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
												WHERE JOBCODEID = '$JP7' AND PRJCODE = '$PRJCODE'";
									$this->db->query($s_u7b);
									...
									foreach($r_P8 as $rw_P8) :
										$JP8 	= $rw_P8->JOBPARENT;
										$s_u8	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
														JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
													WHERE JOBCODEID = '$JP8' AND PRJCODE = '$PRJCODE'";
										$this->db->query($s_u8);

										$s_u8	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
														JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
													WHERE JOBCODEID = '$JP8' AND PRJCODE = '$PRJCODE'";
										$this->db->query($s_u8);

										$s_u8b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
														ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
														, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
													WHERE JOBCODEID = '$JP8' AND PRJCODE = '$PRJCODE'";
										$this->db->query($s_u8b);
										...
										foreach($r_P9 as $rw_P9) :
											$JP9 	= $rw_P9->JOBPARENT;
											$s_u9	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
															JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
														WHERE JOBCODEID = '$JP9' AND PRJCODE = '$PRJCODE'";
											$this->db->query($s_u9);

											$s_u9	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
															JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
														WHERE JOBCODEID = '$JP9' AND PRJCODE = '$PRJCODE'";
											$this->db->query($s_u9);

											$s_u9b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
															ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
															, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
														WHERE JOBCODEID = '$JP9' AND PRJCODE = '$PRJCODE'";
											$this->db->query($s_u9b);
											...
											foreach($r_P10 as $rw_P10) :
												$JP10 	= $rw_P10->JOBPARENT;
												$s_u10	= "UPDATE tbl_boqlist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
																JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
															WHERE JOBCODEID = '$JP10' AND PRJCODE = '$PRJCODE'";
												$this->db->query($s_u10);

												$s_u10	= "UPDATE tbl_joblist SET JOBVOLM = IF(JOBVOLM = 0, 1, JOBVOLM),
																JOBCOST = JOBCOST - $ITM_BUDG, PRICE = JOBCOST / IF(JOBVOLM=0,1,JOBVOLM)
															WHERE JOBCODEID = '$JP10' AND PRJCODE = '$PRJCODE'";
												$this->db->query($s_u10);

												$s_u10b	= "UPDATE tbl_joblist_detail SET ITM_VOLM = IF(ITM_VOLM = 0, 1, ITM_VOLM),
																ITM_BUDG = ITM_BUDG - $ITM_BUDG, ITM_PRICE = ITM_BUDG / ITM_VOLM
																, ITM_LASTP = ITM_BUDG / ITM_VOLM, ITM_AVGP = ITM_BUDG / ITM_VOLM
															WHERE JOBCODEID = '$JP10' AND PRJCODE = '$PRJCODE'";
		...
		echo "Pembatalan komponen RAP sudah kami proses. Silahkan cek perubahannya di daftar RAP..! $s_P2 $s_P3";
    }
