...
class Hrdocument_ska_skt  extends CI_Controller
{
	function __construct() // GOOD
	{
		parent::__construct();

		$this->load->model('m_project/m_hrdocument_ska_skt/m_hrdocument', '', TRUE);
 	...
 	public function index() // OK
	{
		...
		$url			= site_url('c_project/hrdocument_ska_skt/get1nd3x/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function get1nd3x()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data["secURL"] 	= "c_project/hrdocument_ska_skt/get_last_ten_GroupType/?id=";
	...
	function get_last_ten_GroupType()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// -------------------- START : SEARCHING METHOD --------------------
				...
				$data["url_search"] = site_url('c_project/hrdocument_ska_skt/f4n7_5rcH/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$data["MenuCode"] 	= 'MN331';
			$data['backURL'] 	= site_url('c_project/hrdocument_ska_skt/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$this->load->view('v_project/v_hrdocument_ska_skt/hrdocument_type', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function get_last_ten_projList($offset=0) // HOLD
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['secAddURL'] 			= site_url('c_project/hrdocument_ska_skt/add/?id='.$this->url_encryption_helper->encode_url($appName));
			$data['showIndex'] 			= site_url('c_project/hrdocument_ska_skt/index/?id='.$this->url_encryption_helper->encode_url($appName));
			$data['srch_url'] 			= site_url('c_project/hrdocument_ska_skt/get_last_ten_docpattern_src/?id='.$this->url_encryption_helper->encode_url($appName));			
			$data['moffset'] 			= $offset;
			$data['perpage'] 			= 20;
			$data['theoffset']			= 0;
			...
			$config['base_url'] 		= site_url('c_project/hrdocument_ska_skt/get_last_ten_projList');				
			...
			$this->load->view('v_project/v_hrdocument_ska_skt/project_planning', $data);
		}
		else
		{
			redirect('Auth');
	...
	function hr_documentlistx($offset=0) // HOLD
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['h3_title'] 			= 'HR Document';
			$data['main_view'] 			= 'v_project/v_hrdocument_ska_skt/hrdocumentlist';	
					
			$data['link'] 				= array('link_back' => anchor('c_project/hrdocument_ska_skt/','<input type="button" name="btnCancel" id="btnCancel" class="btn btn-danger" value="Back" />', array('style' => 'text-decoration: none;')));
			$data['PRJCODE'] 			= $PRJCODE;
			//$data['moffset'] 			= $offset;
			...
			$data["recordcount"] 		= $num_rows;			
			$config 					= array();
			$config['base_url'] 		= site_url('c_project/hrdocument_ska_skt/get_last_ten_projHRDOC1');
			$config["total_rows"] 		= $num_rows;			
			$config["per_page"] 		= 20;
			...
			$this->load->view('v_project/v_hrdocument_ska_skt/hrdocumentlist', $data);
		}
		else
		{
			redirect('Auth');
	...
	function get_last_ten_GroupType_220707($offset=0) // OK
	{
		$this->load->model('m_project/m_hrdocument_ska_skt/m_hrdocument', '', TRUE);
	...
	function get_last_ten_Type($offset=0) // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$cancel_url				= site_url('c_project/hrdocument_ska_skt/get_last_ten_GroupType/?id='.$this->url_encryption_helper->encode_url($appName));
			...
			$this->load->view('v_project/v_hrdocument_ska_skt/hrdocument', $data);
		}
		else
		{
			redirect('Auth');
	...
	function hr_documentlist() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$cancel_url			= site_url('c_project/hrdocument_ska_skt/get_last_ten_Type/?id='.$this->url_encryption_helper->encode_url($doc_parentA));
  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$secUpd			= site_url('c_project/hrdocument_ska_skt/update/?id='.$this->url_encryption_helper->encode_url($HRDID));
				$secUplURL		= site_url('c_project/hrdocument_ska_skt/hrdocproject_upload/?id='.$this->url_encryption_helper->encode_url($HRDID));
	...
	function add() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$cancel_url					= site_url('c_project/hrdocument_ska_skt/hr_documentlist/?id='.$this->url_encryption_helper->encode_url($DOCCODE));
			$data['backURL'] 			= $cancel_url;
			...
			$data['main_view'] 			= 'v_project/v_hrdocument_ska_skt/hrdocument_form';
			$data['form_action']		= site_url('c_project/hrdocument_ska_skt/do_upload');
			$data['link'] 				= array('link_back' => anchor("$cancel_url",'<input type="button" name="btnCancel" id="btnCancel" class="btn btn-danger" value="Back" />', array('style' => 'text-decoration: none;')));
			
			$MenuCode 					= 'MN234';
			$data['viewDocPattern'] 	= $this->m_hrdocument->getDataDocPat($MenuCode)->result();
			
			$this->load->view('v_project/v_hrdocument_ska_skt/hrdocument_form', $data);
		}
		else
		{
			redirect('Auth');
	...
	function hrdocproject_upload() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$this->load->view('v_project/v_hrdocument_ska_skt/hrdocument_upload', $data);
		}
		else
		{
			redirect('Auth');
	...
	function do_upload() // OK
	{
		...
		$HRDOC_CREATED		= date('Y-m-d H:i:s');
			...
			$pattNo 		= $nol.$myMax;
			$DocNumber 		= "$PATTCODE$DOCCODE-$pattNo";
			$HRDOCNO		= "$PATTCODE$DOCCODE-$pattNo";
		...
		$url			= site_url('c_project/hrdocument_ska_skt/hr_documentlist/?id='.$this->url_encryption_helper->encode_url($DOCCODE));
		redirect($url);
	}
	...
	function getallemp()
	{
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$this->load->view('v_project/v_hrdocument_ska_skt/hrdocument_emp', $data);
		}
		else
		{
			redirect('Auth');
