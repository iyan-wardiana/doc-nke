 ...
 * File Name	= C_gLb53TT1n9.php
 * Location		= -
*/

class C_gLb53TT1n9  extends CI_Controller
{
	var $limit = 2;
	var $title = 'NKE ITSys';
	...
	{		
		...
		$url			= site_url('c_setting/c_gLb53TT1n9/index1/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	public function index1($offset=0)
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['form_action']	= site_url('c_setting/c_gLb53TT1n9/update_process');
			$data['link'] 			= array('link_back' => anchor('c_setting/c_gLb53TT1n9/','<input type="button" name="btnCancel" id="btnCancel" class="button_css" value="Cancel" />', array('style' => 'text-decoration: none;')));
			$data["MenuCode"] 		= 'MN075';
	...
	function update_process()
	{
		...
		if($genSett == 1)
		{
			$globalsetting = array('Display_Rows' 	=> $this->input->post('Display_Rows'), 
							...
							'ACC_ID_IRRET'		=> $this->input->post('ACC_ID_IRRET'),
							'ACC_ID_IRPPN'		=> $this->input->post('ACC_ID_IRPPN'),
							'ACC_ID_PROD'		=> $this->input->post('ACC_ID_PROD'),
							'ACC_ID_UMSUB'		=> $this->input->post('ACC_ID_UMSUB'),
							...
							'ACC_ID_OEXP'		=> $this->input->post('ACC_ID_OEXP'),
							'ACC_ID_MC'			=> $this->input->post('ACC_ID_MC'),
							'ACC_ID_SALRET'		=> $this->input->post('ACC_ID_SALRET'),
							'ACC_ID_WIPP'		=> $this->input->post('ACC_ID_WIPP'),
							...
							'SET_DEPTPURCH'		=> $this->input->post('SET_DEPTPURCH'),
							'APPLEV'			=> $this->input->post('APPLEV')); 
			$this->m_globalsetting->update($globalsetting);

			$data 		= array('SETDPURCH' => $this->input->post('SET_DEPTPURCH'));
