...
class C_bUd93tL15t extends CI_Controller  
{
			...
			// CREATE JOBLIST VIEW
				...
				$s_cv 			= "CREATE VIEW vw_joblist_detail_$PRJCODEVW AS
									SELECT ORD_ID,
										...
										OPN_QTY, OPN_AMOUNT, ITM_USED, ITM_USED_AM, ITM_STOCK, ITM_STOCK_AM, IS_LEVEL, ISLAST, ISLASTH, ISLAST_BOQ
	...
	function up1h0fbt_process() // G
	{
		...
		if($Acc_DirParentB == $Acc_DirParent)
		{
			...
			// SAVE TO ALL PROJECT
				...
				foreach($resPRJ as $rowPRJ) :
					...
					$coaUpd 	= array('Account_Number'		=> $Account_Number,
		...
		else
		{
			...
			// CARI NEXT ORD_ID
				$MAX_ORDID 	= $ORD_ID;
				$sqlMAXID 	= "SELECT MAX(ORD_ID) AS MAX_ORDID, Account_Class
								FROM tbl_chartaccount WHERE Acc_DirParent = '$Acc_DirParent' AND PRJCODE = '$PRJCODE'";
				$resMAXID 	= $this->db->query($sqlMAXID)->result();
				foreach($resMAXID as $rowMAXID) :
					$MAX_ORDIDA 	= $rowMAXID->MAX_ORDID;
					$Account_Class 	= $rowMAXID->Account_Class;
					$MAX_ORDID 		= $MAX_ORDIDA+1;
				endforeach;
				if($MAX_ORDID == 0 || $MAX_ORDID == '')
					$MAX_ORDID 	= 1;

				$s_updORID 	= "UPDATE tbl_chartaccount SET ORD_ID = ORD_ID+1 WHERE ORD_ID >= $MAX_ORDID";
				$this->db->query($s_updORID);
			...
			// SAVE TO ALL PROJECT
				...
				foreach($resPRJ as $rowPRJ) :
					...
					$coaUpd 	= array('Acc_ID'				=> $Account_Number,
										...
										'isLast'				=> $isLast);
					$this->m_coa->updateNEW($Acc_ID, $coaUpd, $PRJCODE);
				endforeach;
    ...
    function svJL()
    {
		...
		{
			// BOQ
				$s_upd01 	= "UPDATE tbl_boqlist SET JOBDESC = '$JOBDESC', JOBUNIT = '$JOBUNIT', ISLASTH = '$ISLASTH',
			...
			// JOBLIST
				$s_upd02 	= "UPDATE tbl_joblist SET JOBDESC = '$JOBDESC', JOBUNIT = '$JOBUNIT', ISLASTH = '$ISLASTH',
								JOBVOLM = '$ITM_VOLM', PRICE = '$ITM_PRICE', JOBCOST = '$ITM_BUDG',
								BOQ_VOLM = '$BOQ_VOLM', BOQ_PRICE = '$BOQ_PRICE', BOQ_JOBCOST = '$BOQ_JOBCOST'
				 				WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
			...
			// JOBLISTDETAIL
				$s_upd03 	= "UPDATE tbl_joblist_detail SET JOBDESC = '$JOBDESC', ITM_UNIT = '$JOBUNIT', ISLASTH = '$ISLASTH',
		...
		else
		{
			// BOQ
				$s_01 		= "SELECT JOBLEV, JOBCODEID, PRJCODE_HO, PRJPERIOD
								FROM tbl_boqlist WHERE JOBCODEID = '$JOBPARENT' AND PRJCODE = '$PRJCODE' LIMIT 1";
				$r_01 		= $this->db->query($s_01)->result();
				foreach($r_01 as $rw_01) :
					$JOBLEV 	= $rw_01->JOBLEV;
					...
					$s_ORIDBQ 	= "SELECT ORD_ID FROM tbl_boqlist WHERE JOBPARENT = '$JOBBQH' AND PRJCODE = '$PRJCODE'
									ORDER BY ORD_ID DESC LIMIT 1";
			...
			// JOBLIST
				...
				foreach($r_02 as $rw_02) :
					...
					$s_ORIDJL 	= "SELECT ORD_ID FROM tbl_joblist WHERE JOBPARENT = '$JOBBQH' AND PRJCODE = '$PRJCODE'
									ORDER BY ORD_ID DESC LIMIT 1";
			...
			// JOBLISTDETAIL
				...
				// GET LAST JOBCODEID DETAIL FROM LAST HEADER
					...
					$PRJHO 		= "";
					$PRJPER 	= $PRJCODE;
					$sqlORD1 	= "SELECT JOBCODEID FROM tbl_joblist_detail WHERE JOBPARENT = '$JOBPARENT' AND PRJCODE = '$PRJCODE'
									ORDER BY ORD_ID DESC LIMIT 1";
					$resORD1 	= $this->db->query($sqlORD1)->result();
					foreach($resORD1 as $rowORD) :
						...
						$sqlORD2 	= "SELECT ORD_ID FROM tbl_joblist_detail WHERE JOBPARENT LIKE '$JOBID1%' AND PRJCODE = '$PRJCODE'
										ORDER BY ORD_ID DESC LIMIT 1;";
