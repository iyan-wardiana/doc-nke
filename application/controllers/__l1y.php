...
class __L1y extends CI_Controller
 {
	...
	function trashPR()
	{
		...
		$PERIODE 		= date('Y-m-d');
		$s_doc 			= "SELECT PR_DATE FROM tbl_pr_header WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
    ...
    function getLastDocNum()
    {
		...
		// START : GET LAST RUN NUMBER
			if($DOCTYPE == 'PR' || $DOCTYPE == 'PO' || $DOCTYPE == 'IR' || $DOCTYPE == 'TTK'  || $DOCTYPE == 'TTKD' || $DOCTYPE == 'VC' || $DOCTYPE == 'VCASH' || $DOCTYPE == 'CHO-PD' || $DOCTYPE == 'DPSPL')
			{
				// START : MANUAL NUMBER KAS/BANK
					$payCode 	= "";
					$ACC_BAL 	= 0;
	...
	function voidDoc_CJRN()			// Void Doc. Create New Doc. Journal
	{
		date_default_timezone_set("Asia/Jakarta");
		...
		*/
		...
		// 1. UPDATE JOURNAL, HANYA KETERANGAN TANPA MERUBAH STATUS MENJADI VOID
				$sqlDELJH	= "UPDATE tbl_journalheader SET JournalH_Desc3 = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', GEJ_STAT = 9, isCanceled = 1,
			...
			$jrnCode2 	= '';
			$Manual_No 	= $DocNum;
	        $sql1 		= "SELECT JournalType, Manual_No FROM tbl_journalheader WHERE JournalH_Code = '$DocNum'";
	        $res1 		= $this->db->query($sql1)->result();
	        foreach($res1 as $rowQRC):
	        	...
	        	if($JournalType == 'IR')
	        	{
			        ...
			        $sqlDel1	= "UPDATE tbl_ir_header SET IR_STAT = 9, ISVOID = 1, VOID_NOTE = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', VOID_DATE = '$DNOW',
	        	...
	        	{
			        $sqlDel1	= "UPDATE tbl_ir_header SET ISVOID = 1, VOID_NOTE = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', VOID_DATE = '$DNOW'
	        	...
	        	{
			        $sqlDel1	= "UPDATE tbl_pinv_header SET ISVOID = 1, VOID_REASON = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', UPDATED = '$DNOW'
									WHERE INV_NUM = '$DocNum'";
			        $this->db->query($sqlDel1);
		
					// UPDATE TTK
						$TTK_NUM		= '';
						$sqlTTKD		= "SELECT IR_NUM AS TTK_NUM FROM tbl_pinv_header WHERE INV_NUM = '$DocNum' LIMIT 1";
						$resTTKD		= $this->db->query($sqlTTKD)->result();
						foreach($resTTKD as $rowTTKD):
							$TTK_NUM	= $rowTTKD->TTK_NUM;
						endforeach;
	        	...
	        	{
			        $sqlDel1	= "UPDATE tbl_journalheader_vcash SET JournalH_Desc3 = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', isCanceled = 1,
			        					LastUpdate = '$DNOW', STATDESC = 'Void', STATCOL = 'danger'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel1);

			        $sqlDel2	= "UPDATE tbl_journaldetail_vcash SET isVoid = 1, oth_reason = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW',
										LastUpdate = '$DNOW'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel2);
	        	...
	        	{
			        $sqlDel1	= "UPDATE tbl_journalheader_cprj SET JournalH_Desc3 = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', isCanceled = 1,
			        					LastUpdate = '$DNOW', STATDESC = 'Void', STATCOL = 'danger'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel1);

			        $sqlDel2	= "UPDATE tbl_journaldetail_cprj SET isVoid = 1, oth_reason = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW',
										LastUpdate = '$DNOW'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel2);
	        	...
	        	{
			        $sqlDel1	= "UPDATE tbl_journalheader_pd SET JournalH_Desc3 = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', isCanceled = 1,
			        					LastUpdate = '$DNOW', STATDESC = 'Void', STATCOL = 'danger'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel1);

			        $sqlDel2	= "UPDATE tbl_journaldetail_pd SET isVoid = 1, oth_reason = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW',
										LastUpdate = '$DNOW'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel2);
	        	...
	        	{
			        $sqlDel1	= "UPDATE tbl_bp_header SET VOID_REASON = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', ISVOID = 1, CB_STAT = 9, STATDESC = 'Void', STATCOL = 'danger'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel1);
	        	...
	        	{
			        $sqlDel1	= "UPDATE tbl_br_header SET VOID_REASON = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', ISVOID = 1, BR_STAT = 9, STATDESC = 'Void', STATCOL = 'danger'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel1);
	        	...
	        	{
			        $sqlDel1	= "UPDATE tbl_journalheader_pb SET JournalH_Desc3 = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW', isCanceled = 1,
			        					LastUpdate = '$DNOW', STATDESC = 'Void', STATCOL = 'danger'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel1);

			        $sqlDel2	= "UPDATE tbl_journaldetail_pb SET isVoid = 1, oth_reason = 'Dibatalkan oleh $voidNM ($EMPID) : $DNOW',
										LastUpdate = '$DNOW'
									WHERE JournalH_Code = '$DocNum'";
			        $this->db->query($sqlDel2);
	        	}
	...
	function cancelItem()
	{
		...
		$JOBDESC 	= "-";
		$s_00 		= "SELECT JOBDESC FROM tbl_joblist_detail WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE' LIMIT 1";
		$r_00 		= $this->db->query($s_00)->result();
		foreach($r_00 as $rw_00) :
		...
		if($V_CATEG == 'PR')
		{
			...
			foreach($r_01 as $rw_01) :
				...
				// PENGEMBALIAN SISA VOLUME KE BUDGET
					$s_03 	= 	"UPDATE tbl_joblist_detail SET REQ_VOLM = REQ_VOLM - $V_VOL, REQ_AMOUNT = REQ_AMOUNT - $PR_CTOTAL
									WHERE JOBCODEID = '$JOBCODEID' AND ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE'";
					$this->db->query($s_03);
				...
				// UPDATE ITEM
					$s_07	= "UPDATE tbl_item SET PR_VOLM = PR_VOLM-$V_VOL, PR_AMOUNT = PR_AMOUNT-$PR_CTOTAL WHERE PRJCODE = '$PRJCODE' AND ITM_CODE = '$ITM_CODE'";
					$this->db->query($s_07);
		...
		else if($V_CATEG == 'WO')
		{
			...
			foreach($r_01 as $rw_01) :
				...
				// PENGEMBALIAN SISA VOLUME KE BUDGET
					$s_03 	= 	"UPDATE tbl_joblist_detail SET WO_QTY = WO_QTY - $V_VOL, WO_AMOUNT = WO_AMOUNT - $WO_CTOTAL
									WHERE JOBCODEID = '$JOBCODEID' AND ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE'";
					$this->db->query($s_03);
    ...
    function lockRAP()
    {
    	...
    	$DOC_EMP1	= $_POST['DOC_EMP1'];
    	$DOC_EMP2	= $_POST['DOC_EMP2'];

    	$colDATA 	= "$PRJCODE - $DOC_REF - $DOC_DATE - $DOC_DESC - $DOC_FILE - $DOC_EMP1";
	    ...
	    $CREATER 	= $this->session->userdata['Emp_ID'];
	   	...
	   	$insDATA	= "INSERT INTO tbl_lockrap (LOCK_NUM, LOCK_DOCREF, LOCK_DOCDATE, PRJCODE, LOCK_DESC, LOCK_EMP1, LOCK_EMP2, LOCK_CREATED, LOCK_CREATER)
							VALUES ('$LOCK_NUM', '$DOC_REF', '$DOC_DATE', '$PRJCODE', '$DOC_DESC', '$DOC_EMP1', '$DOC_EMP2', '$CREATED', '$CREATER')";
		$this->db->query($insDATA);

		$updRAP 	= "UPDATE tbl_joblist_detail SET ISLOCK = 1, LOCKCODE = '$LOCK_NUM', LOCKNO = '$DOC_REF', LOCKDATE = '$DOC_DATE' WHERE PRJCODE = '$PRJCODE'";
		$this->db->query($updRAP);

		$updPRJ 	= "UPDATE tbl_project SET RAP_STAT = 1 WHERE PRJCODE = '$PRJCODE'";
    ...
    {
		$PRJCODE 		= $_POST['PRJCODE'];
		$sql 			= "SELECT RAP_STAT FROM tbl_project WHERE PRJCODE = '$PRJCODE'";
		$result 		= $this->db->query($sql)->result();
		foreach($result as $row) :
	...
	function delDOC()
	{
		...
		$sqlCODE	= "SELECT HRDOCNO, HRDOCCODE, HRDOC_NOTE FROM tbl_hrdoc_header WHERE ID = '$HRDID'";
		$resCODE	= $this->db->query($sqlCODE)->result();
		foreach($resCODE as $rowCODE):
			$HRDOCNO 	= $rowCODE->HRDOCNO;
			$HRDOCCODE 	= $rowCODE->HRDOCCODE;
			$HRDOC_NOTE = $rowCODE->HRDOC_NOTE;
		endforeach;

        $sqlDel		= "DELETE FROM tbl_hrdoc_header WHERE ID = '$HRDID'";
        $this->db->query($sqlDel);

        $LangID 	= $this->session->userdata['LangID'];
		if($LangID == 'IND')
		{
			$alert1	= "No. Dokumen : $HRDOCNO ($HRDOCCODE) telah dihapus.";
		}
		else
		{
			$alert1	= "Document no. $HRDOCNO ($HRDOCCODE) has been deleted.";
		}
		echo "$alert1";
	}
  	...
  	function get_AllDailyWR() // GOOD
	{
		...
		$datePeriod1		= $this->input->post('DWR_DATEX');
		...
		// START : FOR SERVER-SIDE
			...
			$num_rows 		= $this->m_updash->get_AllDataDWRC($DWR_EMPID, $Start_Date, $End_Date, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			...
			$query 			= $this->m_updash->get_AllDataDWRL($DWR_EMPID, $Start_Date, $End_Date, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
                ...
                $isVerifier		= "disabled='disabled' aria-current='disabled'";
				...
				$updID 			= "$secUpdROW~$DWR_ID~$DWR_CODE~$DWR_EMPID";

				$secVerROW 		= base_url().'index.php/__l1y/verDWR/?id=';
				$verID 			= "$secVerROW~$DWR_ID~$DWR_CODE~$DWR_EMPID";

				$secUndoROW 	= base_url().'index.php/__l1y/undoDWR/?id=';
				$undoID 		= "$secUndoROW~$DWR_ID~$DWR_CODE~$DWR_EMPID";
				
				$secDelROW 		= base_url().'index.php/__l1y/delDWR/?id=';
				$delID 			= "$secDelROW~$DWR_ID~$DWR_CODE~$DWR_EMPID";
				...
				$time_now 	= date('H:i:s');
				$disableACT = "";
				// if($DWR_DATE == $date_now && $time_now >= '23:00:00' && $time_now <= '00:00:00') 
				// 	$disableACT = "disabled='disabled' aria-current='disabled'";
				
				if($DWR_STAT == 1 || $DWR_STAT == 4)
				{
					if($DWR_DATE != $date_now)
					{
						$disableACT = "disabled='disabled' aria-current='disabled'";
					}
					else
					{
						if($time_now >= '23:00:00' && $time_now <= '00:00:00')
						{
							$disableACT = "disabled='disabled' aria-current='disabled'";
						}
					}
					...
					$secAction	= 	"<input type='hidden' name='urlUpd".$noU."' id='urlUpd".$noU."' value='".$updID."'>
									<input type='hidden' name='urlDel".$noU."' id='urlDel".$noU."' value='".$delID."'>
								   	<label style='white-space:nowrap'>
								   	<a href='javascript:void(null);' class='btn btn-info btn-xs' ".$disableACT." title='Update' onClick='updRow(".$noU.")'>
										<i class='glyphicon glyphicon-pencil'></i>
								   	</a>
									<a href='javascript:void(null);' class='btn btn-danger btn-xs' ".$disableACT." onClick='delRow(".$noU.")' title='Delete'>
				...
				else
				{
					...
					$secAction	= 	"<input type='hidden' name='urlDel".$noU."' id='urlDel".$noU."' value='".$delID."'>
								   	<label style='white-space:nowrap'>
								   	<a href='javascript:void(null);' class='btn btn-info btn-xs' title='Update' disabled='disabled' aria-current='disabled'>
										<i class='glyphicon glyphicon-pencil'></i>
								   	</a>
									<a href='javascript:void(null);' class='btn btn-danger btn-xs' title='Delete' disabled='disabled' aria-current='disabled'>
										<i class='fa fa-trash-o'></i>
									</a>
									</label>";
				}
	...
	function get_AllEMP()
	{
		...
		if($DefEmp_ID == 'D15040004221' || $DefEmp_ID == 'R95060000118' || $DefEmp_ID == 'D08040000828')
			$ADDQRY = "";
		else
			$ADDQRY = "WHERE Pos_Code IN (SELECT POSS_CODE FROM tbl_position_str WHERE POSS_PARENT LIKE '$POSS_CODE%') OR Emp_ID = '$DefEmp_ID'";
    ...
    function saveDWR()
    {
		...
		else
		{
			$s_00		= "UPDATE tbl_lkh SET DWR_CATEG = '$DWR_CATEG', DWR_DATE = '$DWR_DATE', DWR_DATES = '$DWR_DATES', DWR_DATEE = '$DWR_DATEE', DWR_NOTES = '$DWR_NOTES'
							WHERE DWR_ID = '$DWR_ID' AND DWR_EMPID = '$DWR_EMPID' AND DWR_DATE = '$DWR_DATE'";
		}
		$this->db->query($s_00);
	...
	function getDWR()
	{
        ...
        $s_01 		= "SELECT DWR_ID, DWR_EMPNM, DWR_DATE, DWR_DATES, DWR_DATEE, DWR_CATEG, DWR_NOTES FROM tbl_lkh WHERE DWR_ID = $DWR_ID LIMIT 1";
		$r_01 		= $this->db->query($s_01)->result();
		foreach($r_01 as $rw_01) :
			$DWR_ID 	= $rw_01->DWR_ID;
			...
			echo "$DWR_ID~$DWR_CODE~$DWR_EMPID~$DWR_EMPNM~$DWR_DATE~$DWR_DATES~$DWR_DATEE~$DWR_CATEG~$DWR_NOTES";
		endforeach;
	}
	...
	function verDWR()
	{
        ...
        $sqlDel		= "UPDATE tbl_lkh SET DWR_STAT = 3 WHERE DWR_ID = $DWR_ID AND DWR_EMPID = '$DWR_EMPID'";
        $this->db->query($sqlDel);
	}
	...
	function undoDWR()
	{
        ...
        $sqlDel		= "UPDATE tbl_lkh SET DWR_STAT = 1 WHERE DWR_ID = $DWR_ID AND DWR_EMPID = '$DWR_EMPID'";
        $this->db->query($sqlDel);
	}
	...
	function delDWR()
	{
        ...
        $sqlDel		= "DELETE FROM tbl_lkh WHERE DWR_ID = $DWR_ID AND DWR_EMPID = '$DWR_EMPID'";
        $this->db->query($sqlDel);
	...
	function vwDWRPD()
	{
		...
		$this->load->view('v_hr/v_lkh/v_lkh_report_lkh', $data);
	}
	...
	function vwDWRPD2()
	{
		...
		$this->load->view('v_hr/v_lkh/v_lkh_report_xls', $data);
	}

	function updLKHRes()
	{
		$DWR_EMPID 	= $this->input->post("DWR_EMPID");
		$DWR_DATE 	= $this->input->post("DWR_DATE");
		$LKHRes		= $this->input->post("LKHRes");
		$updLKHRes 	= "UPDATE tbl_lkh SET DWR_LKHRES = '$LKHRes' WHERE DWR_EMPID = '$DWR_EMPID' AND DWR_DATE = '$DWR_DATE'";
		$this->db->query($updLKHRes);
	}

	function getEncryptURL()
	{
		echo json_encode($this->url_encryption_helper->encode_url($this->input->post("id")));
	}
	...
	function getDWR_NUM()
	{
		...
		$data 			= ["DWR_NUM" => $DWR_NUM, "DWR_CODE" => $DWR_CODE];
